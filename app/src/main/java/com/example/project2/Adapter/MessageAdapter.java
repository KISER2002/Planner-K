...
import com.example.project2.Chat.Message;
...
public class MessageAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    private class SentImageHolder extends RecyclerView.ViewHolder { // 보내는 이미지 메세지
        ...
        void bind(Message message) {
            Glide.with(context).load("http://43.201.55.113" + message.getMessage()).override(500, 500).into(imageView);
            // Format the stored timestamp into a readable String using method.
            timeText.setText(message.getCreatedAt());
        }
    }
    ...
    private class ReceivedMessageHolder extends RecyclerView.ViewHolder { // 받는 메세지
        ...
        void bind(Message message) {
            ...
            if(message.getUserProfile().equals("basic_image")){
                ...
                Glide.with(context).load("http://43.201.55.113" + message.getUserProfile()).override(50, 50).into(profileImage);
            }
        }
    }
