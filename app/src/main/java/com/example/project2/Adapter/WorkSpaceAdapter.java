...
public class WorkSpaceAdapter extends RecyclerView.Adapter<WorkSpaceAdapter.ViewHolder> {
    ...
    public WorkSpaceAdapter(Activity context, ArrayList<WorkSpace> chatRoomList, OnListItemSelectedInterface mListener) {
    ...
    @NonNull
    @Override
    public WorkSpaceAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        // 미리 만들어 놓은 item_post.xml 기입
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.workspace_item, parent, false);
        WorkSpaceAdapter.ViewHolder holder = new WorkSpaceAdapter.ViewHolder(view) ;

        return holder;
    }

    @Override
    public void onBindViewHolder(@NonNull WorkSpaceAdapter.ViewHolder holder, int position) {
